...
export default function GroupSessionPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto p-3 sm:p-6 md:p-8 animate-fade-in my-auto">
        <div className="bg-[#532E60] border-2 border-white/40 shadow-2xl rounded-3xl p-5 sm:p-6 md:p-8 text-white space-y-5">
          ...
          <div className="bg-[#613B6E] p-4 sm:p-5 rounded-2xl border border-white/20 space-y-3 text-xs sm:text-sm md:text-base font-bold text-[#E8DCF8] leading-relaxed">
          ...
          <button
            ...
            className="w-full py-3.5 bg-[#C4B4E2] hover:bg-white text-[#532E60] font-black text-sm sm:text-base rounded-full shadow-lg border border-white transition-all cursor-pointer"
          >
  ...
  if (activeSession) {
    return (
      <div className="h-full flex flex-col max-w-4xl mx-auto space-y-3 sm:space-y-4 pb-4">
        
        {/* Group Header Bar */}
        <div className="bg-[#532E60] border-2 border-white/40 shadow-xl rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 text-white flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <Users size={18} className="text-[#C4B4E2] shrink-0" />
              <h2 className="font-serif font-black text-lg sm:text-xl md:text-2xl text-white">{activeSession.title}</h2>
            </div>
            <div className="flex items-center gap-2.5 text-[11px] sm:text-xs text-[#E8DCF8] font-bold mt-0.5">
              <span>Code: <strong className="text-white tracking-wider sm:tracking-widest bg-[#613B6E] px-1.5 py-0.5 rounded border border-white/20">{activeSession.code}</strong></span>
              <span>•</span>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button
              ...
              className="flex items-center gap-1 px-2.5 sm:px-3 py-1.5 bg-[#C4B4E2] text-[#532E60] font-black text-[11px] sm:text-xs rounded-full hover:bg-white transition-all border border-white cursor-pointer"
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 sm:px-3 py-1.5 bg-[#613B6E] text-white font-bold text-[11px] sm:text-xs rounded-full hover:bg-[#6D427C] transition-all border border-white/30 cursor-pointer"
            ...
            {currentParticipant.isCreator ? (
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 bg-red-900/80 hover:bg-red-800 text-red-100 font-black text-[11px] sm:text-xs rounded-full border border-red-500 cursor-pointer"
              >
                <LogOut size={13} />
                <span>End</span>
              </button>
        ...
        <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 px-1">
          {activeSession.participants.map(p => {
            const isMe = p.id === currentParticipant.id;
            return (
              <span 
                key={p.id} 
                className={`px-3 py-1 text-[11px] sm:text-xs font-black rounded-full border transition-all shrink-0 ${
                  isMe 
        ...
        <div className="flex-1 bg-[#532E60] border-2 border-white/40 shadow-2xl rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 overflow-y-auto space-y-3 sm:space-y-4 min-h-[320px]">
          {activeSession.messages.map(msg => {
            ...
            if (isSystem) {
              return (
                <div key={msg.id} className="flex justify-center my-1.5">
                  <span className="px-3 py-1 bg-[#613B6E]/80 text-[#C4B4E2] text-[11px] font-bold rounded-full border border-white/20 shadow-sm text-center">
                    {msg.content}
                  </span>
                </div>
              );
            ...
            return (
              <div key={msg.id} className={`flex ${isAI ? 'justify-start' : isMe ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[85%] sm:max-w-[80%] rounded-2xl sm:rounded-3xl p-3 sm:p-4 shadow-md ${
                  isAI
                  ...
                  <p className="text-[10px] sm:text-[11px] uppercase tracking-wider font-black mb-1 opacity-75">
                    {msg.senderName}
                  </p>
          ...
          {isLoading && (
            <div className="flex justify-start items-center">
              <img src={itouraMascot} alt="Thinking" className="w-8 h-8 sm:w-9 sm:h-9 rounded-full mr-2 sm:mr-3 border border-white/40" />
              <div className="bg-[#613B6E] p-2.5 sm:p-3 rounded-2xl text-xs font-bold text-[#E8DCF8] animate-pulse border border-white/20">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 p-2 sm:p-6 md:p-8 animate-fade-in pb-16">
      <header className="space-y-1 sm:space-y-2 pt-1 sm:pt-0">
        <h1 className="font-serif text-2xl sm:text-3xl md:text-4xl font-black text-[#532E60] leading-tight">{t('group.title')}</h1>
        <p className="text-[#532E60]/80 font-bold text-xs sm:text-sm md:text-base leading-relaxed">{t('group.subtitle')}</p>
      </header>
      ...
      <div className="grid md:grid-cols-2 gap-5 sm:gap-6">
        ...
        <div className="bg-[#532E60] border-2 border-white/40 shadow-2xl rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 text-white space-y-5">
          <div className="flex items-center gap-3">
            <div className="p-2.5 sm:p-3 bg-[#613B6E] rounded-2xl border border-white/30 shrink-0">
              <Plus size={20} className="text-[#C4B4E2]" />
            </div>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              <label className="block text-[11px] sm:text-xs font-black text-[#C4B4E2] uppercase tracking-wider mb-1.5">Display Name</label>
              <input
                type="text"
                required
                value={displayName}
                ...
                className="w-full px-3.5 py-2.5 sm:py-3 bg-[#C4B4E2] text-[#532E60] font-bold text-xs sm:text-base placeholder:text-[#532E60]/60 rounded-xl outline-none border border-white shadow-inner"
              />
            </div>

            <div>
              <label className="block text-[11px] sm:text-xs font-black text-[#C4B4E2] uppercase tracking-wider mb-1.5">Session Title (Optional)</label>
              <input
                type="text"
                value={createTitle}
            ...
            <button
              ...
              className="w-full py-3.5 sm:py-4 bg-[#C4B4E2] hover:bg-white text-[#532E60] font-black text-sm sm:text-base rounded-full shadow-lg border border-white transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
            >
        ...
        <div className="bg-[#532E60] border-2 border-white/40 shadow-2xl rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 text-white space-y-5">
          ...
          <form onSubmit={handleJoin} className="space-y-4">
            ...
            <div>
              <label className="block text-[11px] sm:text-xs font-black text-[#C4B4E2] uppercase tracking-wider mb-1.5">Room Code</label>
              <input
                type="text"
                required
                value={joinCode}
                ...
                className="w-full px-3.5 py-2.5 sm:py-3 bg-[#C4B4E2] text-[#532E60] font-bold text-xs sm:text-base placeholder:text-[#532E60]/60 rounded-xl uppercase tracking-widest outline-none border border-white shadow-inner"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 sm:py-4 bg-[#613B6E] hover:bg-[#6D427C] text-white font-black text-sm sm:text-base rounded-full shadow-lg border border-white/30 transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
            >
